...
function formatRoleActivityDate(value) {
  try {
    return new Intl.DateTimeFormat('es-CO', {
      month: 'short',
      day: 'numeric',
...
function buildRoleMetric(label, value, copy) {
  return `
...
function buildRoleActivityList(items, emptyCopy) {
  if (!items.length) {
...
function renderProfile(state) {
  ...
  if (profile.roleKey === 'artist') {
    roleStats = `
      <div class="stat-box"><span class="eyebrow">Subidas</span><strong>${totals.uploads}</strong></div>
      <div class="stat-box"><span class="eyebrow">Publicadas</span><strong>${totals.uploads}</strong></div>`;
  }
  if (profile.roleKey === 'ambassador') {
    roleStats = `
      <div class="stat-box"><span class="eyebrow">Contactos</span><strong>${totals.ambassadorContacts}</strong></div>
      <div class="stat-box"><span class="eyebrow">Difusión</span><strong>${totals.ambassadorHighlights}</strong></div>`;
  }
  if (profile.roleKey === 'manager') {
    roleStats = `
      <div class="stat-box"><span class="eyebrow">QR listos</span><strong>${totals.eventQrReady}</strong></div>
      <div class="stat-box"><span class="eyebrow">Agenda lista</span><strong>${totals.eventAgendaReady}</strong></div>`;
  }
  if (profile.roleKey === 'scout') {
    roleStats = `
      <div class="stat-box"><span class="eyebrow">Shortlist</span><strong>${totals.scoutShortlist}</strong></div>
      <div class="stat-box"><span class="eyebrow">Contactos</span><strong>${totals.scoutContacts}</strong></div>`;
  }
  overview.innerHTML = `
...
function renderRoleDemoSpotlight(state) {
  const mount = $('[data-role-demo-spotlight]');
  if (!mount) return;
  const profile = profileFor(state.profileId);
  if (!profile.roleKey) {
    mount.className = 'panel-card role-demo-spotlight hidden';
    mount.innerHTML = '';
    return;
  }
  const studio = getRoleStudio(state);
  const totals = getRoleActivityTotals(state);
  const activeArtist = selectedArtist(state);
  const activeEvent = eventFor(state.activeEventId);
  mount.className = 'panel-card role-demo-spotlight';

  if (profile.roleKey === 'artist') {
    const recentUploads = studio.artistUploads.slice(0, 3).map((upload) => ({
      title: upload.title,
      copy: `${upload.fileName} · ${upload.sizeLabel} · ${formatRoleActivityDate(upload.createdAt)}`,
      pill: upload.status
    }));
    mount.innerHTML = `
      <div class="role-demo-shell role-demo-shell--spotlight" data-role-demo-scope>
        <div class="role-demo-head">
          <div class="role-demo-copy">
            <p class="eyebrow">Rol especial demo</p>
            <h2 class="section-title">Estudio de artista</h2>
            <p class="helper-copy">Sube música en segundos y deja tus lanzamientos listos para mostrar dentro del demo.</p>
          </div>
          <span class="mini-pill">${escapeHtml(profile.name)}</span>
        </div>
        <div class="role-demo-kpis">
          ${buildRoleMetric('Subidas', totals.uploads, totals.uploads ? 'catálogo demo activo' : 'sin archivos todavía')}
          ${buildRoleMetric('Perfil', profile.role, 'rol especial visible')}
          ${buildRoleMetric('Now playing', activeArtist.name, activeArtist.track)}
        </div>
        <div class="role-quick-actions">
          <input class="role-file-input" type="file" accept="audio/*" data-role-upload-input>
          <button class="primary-btn upload-music-btn" type="button" data-trigger-role-upload><span>Subir música</span><small>Audio local · demo</small></button>
          <button class="soft-btn" type="button" data-view-target="profile">Abrir estudio</button>
        </div>
        ${buildRoleActivityList(recentUploads, 'Todavía no has subido archivos en este demo.')}
      </div>`;
    return;
  }
  ...
  if (profile.roleKey === 'ambassador') {
    const activity = [
      ...studio.ambassadorContacts.map((entry) => ({
        title: `Contacto enviado a ${artistFor(entry.artistId).name}`,
        copy: `${artistFor(entry.artistId).city} · ${formatRoleActivityDate(entry.createdAt)}`,
        pill: 'Contacto',
        createdAt: entry.createdAt
      })),
      ...studio.ambassadorHighlights.map((entry) => ({
        title: `Difusión activada para ${artistFor(entry.artistId).name}`,
        copy: `${artistFor(entry.artistId).genre} · ${formatRoleActivityDate(entry.createdAt)}`,
        pill: 'Difusión',
        createdAt: entry.createdAt
      }))
    ].sort((left, right) => new Date(right.createdAt) - new Date(left.createdAt)).slice(0, 3);
    mount.innerHTML = `
      <div class="role-demo-shell role-demo-shell--spotlight">
        <div class="role-demo-head">
          <div class="role-demo-copy">
            <p class="eyebrow">Rol especial demo</p>
            <h2 class="section-title">Centro de embajadora</h2>
            <p class="helper-copy">Conecta artistas, mueve difusión y deja la comunidad activada en pocos clics.</p>
          </div>
          <span class="mini-pill">${escapeHtml(profile.name)}</span>
        </div>
        <div class="role-demo-kpis">
          ${buildRoleMetric('Contactos', totals.ambassadorContacts, 'artistas contactados')}
          ${buildRoleMetric('Difusión', totals.ambassadorHighlights, 'activaciones listas')}
          ${buildRoleMetric('Siguiente paso', activeArtist.name, activeArtist.city)}
        </div>
        <div class="role-quick-actions">
          <button class="primary-btn" type="button" data-ambassador-contact="${escapeHtml(activeArtist.id)}">Contactar artista activo</button>
          <button class="soft-btn" type="button" data-view-target="community">Ir a comunidad</button>
        </div>
        ${buildRoleActivityList(activity, 'Aún no has activado contactos ni difusión.')}
      </div>`;
    return;
  }

  if (profile.roleKey === 'manager') {
    const operations = Object.entries(studio.eventOps || {}).map(([eventId, entry]) => ({
      eventId,
      entry,
      createdAt: entry.updatedAt
    })).sort((left, right) => new Date(right.createdAt) - new Date(left.createdAt)).slice(0, 3).map(({ eventId, entry }) => {
      const eventItem = eventFor(eventId);
      const states = [entry.qrReady ? 'QR listo' : '', entry.agendaReady ? 'Agenda lista' : ''].filter(Boolean).join(' · ') || 'Pendiente';
      return {
        title: eventItem.name,
        copy: `${states} · ${formatRoleActivityDate(entry.updatedAt)}`,
        pill: eventItem.place
      };
    });
    mount.innerHTML = `
      <div class="role-demo-shell role-demo-shell--spotlight">
        <div class="role-demo-head">
          <div class="role-demo-copy">
            <p class="eyebrow">Rol especial demo</p>
            <h2 class="section-title">Operación de eventos</h2>
            <p class="helper-copy">Activa QR, confirma agenda y deja los eventos listos para la presentación.</p>
          </div>
          <span class="mini-pill">${escapeHtml(profile.name)}</span>
        </div>
        <div class="role-demo-kpis">
          ${buildRoleMetric('QR listos', totals.eventQrReady, 'check-in activo')}
          ${buildRoleMetric('Agenda lista', totals.eventAgendaReady, 'operación confirmada')}
          ${buildRoleMetric('Evento activo', activeEvent.name, activeEvent.time)}
        </div>
        <div class="role-quick-actions">
          <button class="primary-btn" type="button" data-event-ops-qr="${escapeHtml(activeEvent.id)}">Activar QR del evento activo</button>
          <button class="soft-btn" type="button" data-view-target="events">Ir a eventos</button>
        </div>
        ${buildRoleActivityList(operations, 'Todavía no hay operaciones activadas para eventos.')}
      </div>`;
    return;
  }

  const scoutActivity = [
    ...studio.scoutShortlist.map((entry) => ({
      title: `${artistFor(entry.artistId).name} entró a shortlist`,
      copy: `${artistFor(entry.artistId).genre} · ${formatRoleActivityDate(entry.createdAt)}`,
      pill: 'Shortlist',
      createdAt: entry.createdAt
    })),
    ...studio.scoutContacts.map((entry) => ({
      title: `Contacto enviado a ${artistFor(entry.artistId).name}`,
      copy: `${artistFor(entry.artistId).city} · ${formatRoleActivityDate(entry.createdAt)}`,
      pill: 'A&R',
      createdAt: entry.createdAt
  ...
  mount.innerHTML = `
    <div class="role-demo-shell role-demo-shell--spotlight">
      <div class="role-demo-head">
        <div class="role-demo-copy">
          <p class="eyebrow">Rol especial demo</p>
          <h2 class="section-title">Scouting discográfico</h2>
          <p class="helper-copy">Filtra talento, arma shortlist y deja el contacto listo para la demo.</p>
        </div>
        <span class="mini-pill">${escapeHtml(profile.name)}</span>
      </div>
      <div class="role-demo-kpis">
        ${buildRoleMetric('Shortlist', totals.scoutShortlist, 'talentos priorizados')}
        ${buildRoleMetric('Contactos', totals.scoutContacts, 'seguimientos enviados')}
        ${buildRoleMetric('Radar', activeArtist.name, activeArtist.genre)}
      </div>
      <div class="role-quick-actions">
        <button class="primary-btn" type="button" data-scout-shortlist="${escapeHtml(activeArtist.id)}">Agregar artista activo</button>
        <button class="soft-btn" type="button" data-view-target="community">Ir a comunidad</button>
      </div>
      ${buildRoleActivityList(scoutActivity, 'Aún no has guardado artistas en shortlist ni enviado contactos.')}
    </div>`;
}

function renderRoleDemoWorkspace(state) {
  const mount = $('[data-role-demo-workspace]');
  if (!mount) return;
  const profile = profileFor(state.profileId);
  if (!profile.roleKey) {
    mount.className = 'panel-card role-demo-workspace hidden';
    mount.innerHTML = '';
    return;
  }
  const studio = getRoleStudio(state);
  const totals = getRoleActivityTotals(state);
  mount.className = 'panel-card role-demo-workspace';

  if (profile.roleKey === 'artist') {
    const uploads = studio.artistUploads.map((upload) => ({
      title: upload.title,
      copy: `${upload.fileName} · ${upload.sizeLabel} · ${formatRoleActivityDate(upload.createdAt)}`,
      pill: upload.status
    }));
    mount.innerHTML = `
      <div class="role-demo-shell" data-role-demo-scope>
        <div class="role-demo-head">
          <div class="role-demo-copy">
            <p class="eyebrow">Herramienta especial</p>
            <h2 class="section-title">Subida de música</h2>
            <p class="helper-copy">Sube un archivo de audio y publícalo en la plataforma demo sin tocar el resto de la app.</p>
          </div>
          <span class="mini-pill">${totals.uploads} archivos</span>
        </div>
        <div class="role-upload-band">
          <input class="role-file-input" type="file" accept="audio/*" data-role-upload-input>
          <div class="role-upload-copy">
            <strong>Publicación rápida para localhost</strong>
            <span>El archivo queda registrado como lanzamiento demo y visible en este apartado.</span>
          </div>
          <button class="primary-btn upload-music-btn" type="button" data-trigger-role-upload><span>Subir música</span><small>MP3 · WAV · audio local</small></button>
        </div>
        <div class="role-section-block">
          <div class="panel-head">
            <strong>Subidas recientes</strong>
            <span class="mini-pill">demo local</span>
          </div>
          ${buildRoleActivityList(uploads, 'Todavía no has subido música en este perfil.')}
        </div>
      </div>`;
    return;
  }
  ...
  if (profile.roleKey === 'ambassador') {
    const contacts = new Set(studio.ambassadorContacts.map((entry) => entry.artistId));
    const highlights = new Set(studio.ambassadorHighlights.map((entry) => entry.artistId));
    const activity = [
      ...studio.ambassadorContacts.map((entry) => ({
        title: `Contacto enviado a ${artistFor(entry.artistId).name}`,
        copy: `${artistFor(entry.artistId).city} · ${formatRoleActivityDate(entry.createdAt)}`,
    ...
    mount.innerHTML = `
      <div class="role-demo-shell">
        <div class="role-demo-head">
          <div class="role-demo-copy">
            <p class="eyebrow">Herramienta especial</p>
            <h2 class="section-title">Contactos y difusión</h2>
            <p class="helper-copy">Contacta artistas y activa la difusión comunitaria con accesos rápidos para la presentación.</p>
          </div>
          <span class="mini-pill">${totals.ambassadorContacts} contactos</span>
        </div>
          ...
          ${data.artists.slice(0, 4).map((artist) => `
            <article class="role-action-card">
              <div>
                ...
                <span>${escapeHtml(artist.genre)} · ${escapeHtml(artist.city)}</span>
              </div>
              <div class="role-inline-tags">
                ${artist.tags.slice(0, 2).map((tag) => `<span class="tag-chip">${escapeHtml(tag)}</span>`).join('')}
              </div>
              <div class="card-actions">
                <button class="soft-btn small-btn" type="button" data-ambassador-contact="${escapeHtml(artist.id)}" ${contacts.has(artist.id) ? 'disabled' : ''}>${contacts.has(artist.id) ? 'Contactado' : 'Contactar'}</button>
                <button class="primary-btn small-btn" type="button" data-ambassador-highlight="${escapeHtml(artist.id)}" ${highlights.has(artist.id) ? 'disabled' : ''}>${highlights.has(artist.id) ? 'Difusión activa' : 'Activar difusión'}</button>
              </div>
            </article>`).join('')}
        </div>
        <div class="role-section-block">
          <div class="panel-head">
            ...
            <span class="mini-pill">${totals.ambassadorHighlights} campañas</span>
          </div>
          ${buildRoleActivityList(activity, 'Todavía no hay contactos ni campañas activas.')}
        </div>
      </div>`;
    return;
  }
    ...
    mount.innerHTML = `
      <div class="role-demo-shell">
        <div class="role-demo-head">
          <div class="role-demo-copy">
            <p class="eyebrow">Herramienta especial</p>
            <h2 class="section-title">Panel de operación</h2>
            <p class="helper-copy">Activa el QR y confirma la agenda de cada evento sin alterar el flujo de los usuarios comunes.</p>
          </div>
          <span class="mini-pill">${totals.eventQrReady} QR listos</span>
        </div>
        <div class="role-action-grid role-action-grid--events">
          ${data.events.map((item) => {
            const entry = studio.eventOps[item.id] || {};
            return `
              <article class="role-action-card">
                <div>
                  <strong>${escapeHtml(item.name)}</strong>
                  <span>${escapeHtml(item.place)} · ${escapeHtml(item.time)}</span>
                </div>
                <div class="role-inline-tags">
                  <span class="mini-pill">${entry.qrReady ? 'QR listo' : 'QR pendiente'}</span>
                  <span class="mini-pill">${entry.agendaReady ? 'Agenda lista' : 'Agenda pendiente'}</span>
                </div>
                <div class="card-actions">
                  <button class="soft-btn small-btn" type="button" data-event-ops-qr="${escapeHtml(item.id)}" ${entry.qrReady ? 'disabled' : ''}>${entry.qrReady ? 'QR activo' : 'Activar QR'}</button>
                  <button class="primary-btn small-btn" type="button" data-event-ops-agenda="${escapeHtml(item.id)}" ${entry.agendaReady ? 'disabled' : ''}>${entry.agendaReady ? 'Agenda confirmada' : 'Confirmar agenda'}</button>
                  <button class="soft-btn small-btn" type="button" data-open-scanner-btn="${escapeHtml(item.id)}">Ver QR</button>
                </div>
              </article>`;
          }).join('')}
        </div>
  ...
  const shortlist = new Set(studio.scoutShortlist.map((entry) => entry.artistId));
  const contacts = new Set(studio.scoutContacts.map((entry) => entry.artistId));
  const activity = [
    ...studio.scoutShortlist.map((entry) => ({
      title: `${artistFor(entry.artistId).name} agregado a shortlist`,
      copy: `${artistFor(entry.artistId).genre} · ${formatRoleActivityDate(entry.createdAt)}`,
      pill: 'Shortlist',
      createdAt: entry.createdAt
    })),
  ...
  mount.innerHTML = `
    <div class="role-demo-shell">
      <div class="role-demo-head">
        <div class="role-demo-copy">
          <p class="eyebrow">Herramienta especial</p>
          <h2 class="section-title">Scouting y shortlist</h2>
          <p class="helper-copy">Filtra artistas, prioriza talento y deja los contactos listos sin tocar la experiencia de oyentes.</p>
        </div>
        <span class="mini-pill">${totals.scoutShortlist} en radar</span>
      </div>
      <div class="role-action-grid">
        ${data.artists.slice(0, 4).map((artist) => `
          <article class="role-action-card">
            ...
            <div class="card-actions">
              <button class="soft-btn small-btn" type="button" data-scout-shortlist="${escapeHtml(artist.id)}" ${shortlist.has(artist.id) ? 'disabled' : ''}>${shortlist.has(artist.id) ? 'En shortlist' : 'Shortlist'}</button>
              <button class="primary-btn small-btn" type="button" data-scout-contact="${escapeHtml(artist.id)}" ${contacts.has(artist.id) ? 'disabled' : ''}>${contacts.has(artist.id) ? 'Contactado' : 'Contactar'}</button>
            </div>
          </article>`).join('')}
      </div>
      <div class="role-section-block">
        <div class="panel-head">
          <strong>Actividad reciente</strong>
          <span class="mini-pill">${totals.scoutContacts} contactos</span>
        </div>
        ${buildRoleActivityList(activity, 'Todavía no hay talento guardado ni contactos enviados.')}
      </div>
    </div>`;
}
...
function renderAll() {
  ...
  renderProfile(state);
  renderRoleDemoSpotlight(state);
  renderRoleDemoWorkspace(state);
